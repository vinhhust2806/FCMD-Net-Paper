...
test_gen = train_generator(df_test,1,
                                dict(),
                                target_size=IMAGE_SIZE)
results = model.evaluate(test_gen, steps=len(df_test))
...
for i in range(len(df_test)):
    ...
    plt.subplot(1,3,2)
    plt.imshow(np.squeeze(cv2.resize(cv2.imread(df_test['mask_path'].iloc[i]),(256,256))))
